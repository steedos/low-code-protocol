...
- **reference_limit**: 选项显示数量，当显示为下拉选项时，默认会列出10个选项供选择，只有“相关表”、“主表/子表”和“选择框”字段类型才支持此属性。
- **showIcon**: 是否显示图标，每个对象上都可以配置图标，默认情况下相关表字段会在选项上显示引用对象的图标，配置该属性值为false可以隐藏选项上的图标，只有“相关表”、“主表/子表”和“选择框”字段类型才支持此属性。

#### 公式相关属性说明

- **formula**: 公式，通过用户定义的算法自动计算字段的值，只有“公式”字段类型才支持此属性。
...
- **min**: 最小值，定义数值类型的字段允许的最小值，只有“数值”、“金额”和“百分比”字段类型才支持此属性。
- **max**: 最大值，定义数值类型的字段允许的最大值, 只有“数值”、“金额”和“百分比”字段类型才支持此属性。
