...
- **precision**: 精度(数字长度)，定义数值类型的字段的精度。
- **scale**: 小数位数，定义数值类型的字段的小数位数。
- **rows**: 多行文本行数， 定义“长文本”字段类型显示的文本框行数。
- **options**: 选择项，定义“选择框”字段类型的下拉选项。
- **options.$**: 选择项
- **options.$.label**: 显示名
- **options.$.value**: 选项值
- **options.$.color**: 背景颜色
...
- **summary_field**: 要聚合的字段，定义汇总时需要聚合汇总哪个字段，比如汇总合同金额。
- **summary_filters**: 过滤条件，定义汇总时需要汇总哪些记录，只汇总符合该过滤条件的记录。
