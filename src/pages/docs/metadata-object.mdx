...
- **searchable**: 可全局搜索字段，默认为false，表示在对象快速搜索及全局搜索功能中不可查询该字段，设置为true表示可以全局搜索。
- **filterable**: 高级查找默认字段，默认为true，表示在高级查找框中显示为默认字段，如果设置为false表示会折叠显示到高级栏中。需要注意的，该字段功能依赖了 searchable 属性，字段的 searchable 属性值必须为 true 时 filterable 属性值 true 才能生效。
...
| reference_to_field | lookup | 引用的对象的字段，按照此字段去查找相关记录;按此字段保存到数据库 | lookup/master_detail |
...
| reference_limit | number | 数据显示数量 | lookup/master_detail |
| defaultIcon | string | 默认的图标 | lookup/master_detail |
...
| min | number | 最小值 | number/currency |
| max | number | 最大值 | number/currency |
...
- **reference_to_field**: 引用对象保存并查找的值，只有“相关表”和“主表/子表”字段类型才显示此属性。
- **multiple**: 多选，启用后可以保存多个值，只有“相关表”、“主表/子表”和“选择框”字段类型才显示此属性。
...
- **options**: 选择项，定义“选择框”字段类型的下拉选项，只有“选择框”字段类型才显示此属性。。。
- **options.$**: 选择项
- **options.$.label**: 显示名
- **options.$.value**: 选项值
- **options.$.color**: 背景颜色
...
- **formula**: 公式，通过用户定义的算法自动计算字段的值，只有“公式”字段类型才显示此属性。
- **data_type**: 数据类型，保存到数据库中的值类型，只有“公式”和“选择框”字段类型才显示此属性。
- **formula_blank_value**: 空白字段处理, zeroes表示将空白字段视为零，blanks表示将空白字段视为空白，默认为zeroes，只有“公式”才显示此属性。

#### 汇总相关属性说明

- **summary_object**: 要汇总的对象, 只有“累计汇总”字段类型才显示此属性。
- **summary_type**: 汇总类型, 含有COUNT、SUM、MIN、MAX、AVG这五个选择项, 只有“累计汇总”字段类型才显示此属性。
- **summary_field**: 要聚合的字段，定义汇总时需要聚合汇总哪个字段，比如汇总合同金额, 只有“累计汇总”字段类型才显示此属性。
- **summary_filters**: 过滤条件，定义汇总时需要汇总哪些记录，只汇总符合该过滤条件的记录, 只有“累计汇总”字段类型才显示此属性。
...
- **precision**: 精度(数字长度)，定义数值类型的字段的精度, 只有“数值”、“金额”和“百分比”字段类型才显示此属性。
- **scale**: 小数位数，定义数值类型的字段的小数位数, 只有“数值”、“金额”和“百分比”字段类型才显示此属性。

#### 其他相关属性说明

- **rows**: 多行文本行数， 定义“长文本”字段类型显示的文本框行数。
