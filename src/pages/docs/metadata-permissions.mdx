...
| password_history | string | 强制密码历史 |
| max_login_attempts | string | 最大无效登录尝试次数 |
| lockout_interval | string | 锁定有效期间 |
| login_expiration_in_days | number | 登录到期时间 |
| phone_login_expiration_in_days | number | 手机App登录到期时间 |
| logout_other_clients | boolean | 单例登录 |
| phone_logout_other_clients | boolean | 手机App单例登录 |
| enable_MFA | boolean | 多重身份验证 |
